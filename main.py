...
def adicionar():
    ...
    cursor.execute(f'''
    INSERT INTO 
        Catalogo (Nome, Idade, Jogadores, Editora, Obs)
    VALUES(
...
def procurar():
    ...
    cursor.execute(f'''
    SELECT 
        Id, Nome, Idade, Jogadores, Editora, Obs        
    FROM 
        Catalogo 
    WHERE
        Id='{EntId.get()}' OR
    ...
    EntEdit.delete('0', 'end')
    EntDesc.delete('1.0', 'end')
    EntId.insert(0, '{}'.format(busca[0]))
    EntNome.insert(0,'{}'.format(busca[1]))
    EntIdade.insert(0, '{} ANOS'.format(busca[2]))
    EntJog.insert(0, '{} JOGADORES'.format(busca[3]))
...
def jan2():
    ...
    canvas1.create_image(350, 250, image=Bg3)
    ...
    tree.heading('2', text='Classificação Etária')
    tree.heading('3', text='Número de Jogadores')
    tree.heading('4', text='Editora')
    tree.place(x=135, y=80, width=600, height=325)
